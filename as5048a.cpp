...
word AS5048A::read(word registerAddress){
  ...
  //Send the command
    ...
    //Serial.println((( ( read_left_byte & 0xFF ) << 8 ) | ( read_right_byte & 0xFF )) & ~0xC000);
    //delay(200);
  ...
  if (a1_left_byte & 0x40) {
    //Serial.println("Setting A1 Error bit");
    errorFlag = true;    
  }
  else {
    errorFlag = false;
  ...
  //Check if the error bit is set
  if (a2_left_byte & 0x40) {
    //Serial.println("Setting A2 Error bit");
    errorFlag = true;    
  }
  else {
    errorFlag = false;
